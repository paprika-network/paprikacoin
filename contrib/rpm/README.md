...
The RPM spec file provided here is for Paprikacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://paprikacoin.paprikaex.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Paprikacoin git repository](https://github.com/JustAResearcher/Paprikacoin) but are not part of
...
Currently only `Source10` is used, for the example `paprikacoin.conf` file.
...
in the paprikacoin git repository. At present only an SVG version of the Paprikacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the paprikacoin
...
needed when 0.12.1 is released, a proper fix is already in the Paprikacoin git
master branch.
...
idea to build Paprikacoin Core against a static version of that library compiled
...
The `paprikacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `paprikacoin-server` package. At this point, most RPM based
...
The `paprikacoin-server` package should have SELinux support. How to properly do
...
Paprikacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/paprikacoin/). He has been
packaging Paprikacoin for Fedora at least since 2012.
...
4. I renamed the `paprikacoin` package that contains the Qt GUI to `paprikacoin-core` as
that appears to be how the general population refers to it, in contrast to
`paprikacoin-xt` or `paprikacoin-classic`. I wanted to make sure the general population

...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Paprikacoin address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The paprikacoin address for the private key\n"
            "\nResult:\n"
    ...
    return CPaprikacoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to paprikacoind)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/JustAResearcher/Paprikacoin/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Paprikacoin %s\n", CLIENT_BUILD);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                CPaprikacoinExtKey b58extkey;
                b58extkey.SetKey(masterKey);

                CExtPubKey pubkey;
                pubkey = masterKey.Neuter();

                CPaprikacoinExtPubKey b58extpubkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CPaprikacoinSecret(key).ToString(), strTime);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                // Get the Paprikacoin Ext Key from the master key
                CPaprikacoinExtKey b58extkey;
                b58extkey.SetKey(masterKey);
                ...
                // Get the Paprikacoin Ext Key from the public key
                CPaprikacoinExtPubKey b58extpubkey;
                b58extpubkey.SetKey(pubkey);
                ...
                // Create the Paprikacoin Account Ext Private Key
                CPaprikacoinExtKey b58accountextprivatekey;
                b58accountextprivatekey.SetKey(accountKey);

                // Create the Paprikacoin Account Ext Public Key
                CPaprikacoinExtPubKey b58actextpubkey;
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see paprikacoind log for details) and could "

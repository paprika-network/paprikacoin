...
#include "paprikacoingui.h"

#include "paprikacoinunits.h"
...
const std::string PaprikacoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString PaprikacoinGUI::DEFAULT_WALLET = "~Default";

PaprikacoinGUI::PaprikacoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void PaprikacoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a paprikacoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and paprikacoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to PAPRY addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your paprikacoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified paprikacoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a Paprikacoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible paprikacoin command-line options").arg(tr(PACKAGE_NAME)));
...
void PaprikacoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void PaprikacoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** PAPRIKA START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/paprikacoincointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* paprikacoinLabelLayout = new QVBoxLayout(toolbarWidget);
        paprikacoinLabelLayout->addWidget(label);
        paprikacoinLabelLayout->addWidget(toolbar);
        paprikacoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        paprikacoinLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Paprikacoin Market Price"));
        ...
        labelBtcNeox->setText("BTC / PAPRY");
        ...
        // Get the latest Paprikacoin release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void PaprikacoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool PaprikacoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool PaprikacoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void PaprikacoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PaprikacoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void PaprikacoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void PaprikacoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void PaprikacoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void PaprikacoinGUI::optionsClicked()
{
...
void PaprikacoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void PaprikacoinGUI::showDebugWindow()
{
...
void PaprikacoinGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void PaprikacoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void PaprikacoinGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void PaprikacoinGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void PaprikacoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void PaprikacoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void PaprikacoinGUI::openClicked()
{
...
void PaprikacoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PaprikacoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void PaprikacoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void PaprikacoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PaprikacoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PaprikacoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** PAPRIKA START */
void PaprikacoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void PaprikacoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void PaprikacoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void PaprikacoinGUI::gotoRestrictedAssetsPage()
{
...
void PaprikacoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to paprikacoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void PaprikacoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void PaprikacoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void PaprikacoinGUI::updateHeadersSyncProgressLabel()
{
...
void PaprikacoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void PaprikacoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Paprikacoin"); // default title
...
void PaprikacoinGUI::changeEvent(QEvent *e)
{
...
void PaprikacoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PaprikacoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PaprikacoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "PAPRY")
        msg += tr("Amount: %1\n").arg(PaprikacoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(PaprikacoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void PaprikacoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PaprikacoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PaprikacoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PaprikacoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PaprikacoinGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void PaprikacoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void PaprikacoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PaprikacoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PaprikacoinGUI::detectShutdown()
{
...
void PaprikacoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void PaprikacoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void PaprikacoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(PaprikacoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(PaprikacoinGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void PaprikacoinGUI::subscribeToCoreSignals()
{
...
void PaprikacoinGUI::unsubscribeFromCoreSignals()
{
...
void PaprikacoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void PaprikacoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<PaprikacoinUnits::Unit> units = PaprikacoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const PaprikacoinUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(PaprikacoinUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(PaprikacoinUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (PaprikacoinUnits::Unit u : PaprikacoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(PaprikacoinUnits::name(u)), this);
...
{
    setText(PaprikacoinUnits::name(newUnits));
}
...
void PaprikacoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=PAPRYBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void PaprikacoinGUI::mnemonic()
{
...
void PaprikacoinGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/JustAResearcher/Paprikacoin/releases"));

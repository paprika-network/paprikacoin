...
#include "paprikacoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a paprikacoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new PaprikacoinAddressEntryValidator(parent));
    widget->setCheckValidator(new PaprikacoinAddressCheckValidator(parent));
}
...
bool parsePaprikacoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no paprikacoin: URI
    if(!uri.isValid() || uri.scheme() != QString("paprikacoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!PaprikacoinUnits::parse(PaprikacoinUnits::PAPRY, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsePaprikacoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert paprikacoin:// to paprikacoin:
    //
    //    Cannot handle this later, because paprikacoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("paprikacoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "paprikacoin:");
    }
    QUrl uriInstance(uri);
    return parsePaprikacoinURI(uriInstance, out);
}

QString formatPaprikacoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("paprikacoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PaprikacoinUnits::format(PaprikacoinUnits::PAPRY, info.amount, false, PaprikacoinUnits::separatorNever));
        paramCount++;
    }
...
bool openPaprikacoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", PAPRIKA_CONF_FILENAME));

    /* Open paprikacoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Paprikacoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Paprikacoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Paprikacoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Paprikacoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "paprikacoin.desktop";
    return GetAutostartDir() / strprintf("paprikacoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a paprikacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Paprikacoin\n";
        else
            optionFile << strprintf("Name=Paprikacoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the paprikacoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef paprikacoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (paprikacoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, paprikacoinAppUrl);

    CFRelease(paprikacoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add paprikacoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, paprikacoinAppUrl, nullptr, nullptr);
    }

...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** PAPRIKA START */
    case OP_PAPRY_ASSET              : return "OP_PAPRY_ASSET";
    /** PAPRIKA END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_PAPRY_ASSET) { // OP_PAPRY_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == PAPRY_N) { // Check to see if PAPRY starts at 27 ( this->size() < 105)
                if ((*this)[28] == PAPRY_E)
                    if ((*this)[29] == PAPRY_X)
                        index = 30;
            } else {
                if ((*this)[28] == PAPRY_N) // Check to see if PAPRY starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == PAPRY_E)
                        if ((*this)[30] == PAPRY_X)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == PAPRY_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == PAPRY_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == PAPRY_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == PAPRY_N) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_PAPRY_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_PAPRY_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_PAPRY_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_PAPRY_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_PAPRY_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_PAPRY_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}

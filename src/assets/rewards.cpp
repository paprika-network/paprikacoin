...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for PAPRY
    ...
    if (p_rewardSnapshot.strDistributionAsset != "PAPRY") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is PAPRY with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using PAPRY differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "PAPRY") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Paprikacoin address (already validated during ownership snapshot creation)

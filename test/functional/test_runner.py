...
`test/functional/test_framework/test_framework.py:PaprikacoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/paprikacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    logging.debug("Temporary test directory at %s" % tmpdir)

    # Don't run tests on Windows by default
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/JustAResearcher/Paprikacoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/Bitcoin/bitcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    # Check that the build was configured with wallet, utils, and paprikacoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_paprikacoind = config["components"].getboolean("ENABLE_PAPRIKAD")
    if not (enable_wallet and enable_cli and enable_paprikacoind):
        print("No functional tests to run. Wallet, utils, and paprikacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if paprikacoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "paprikacoind"]) is not None:
            print("%sWARNING!%s There is already a paprikacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "PAPRIKAD" not in os.environ:
        os.environ["PAPRIKAD"] = build_dir + '/src/paprikacoind' + exeext
        os.environ["PAPRIKACLI"] = build_dir + '/src/paprikacoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `paprikacoin-cli help` (`rpc_interface.txt`).
